define :nginx_web_app, :template => "site.erb", :enable => true do
  ...
  if application[:ssl_support]
    execute "generating dhparams" do
      command "/usr/bin/openssl dhparam #{node[:nginx][:ssl_dhparam_bits]} -out #{node[:nginx][:dir]}/ssl/dhparam.pem"
      not_if { ::File.exists?("#{node[:nginx][:dir]}/ssl/dhparam.pem") }
      action :run
    end
  end
